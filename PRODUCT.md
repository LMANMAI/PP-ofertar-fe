...
- Backend real en Spring Boot + MySQL (repo separado `PP-ofertar`); frontend en Expo + React Native + TypeScript (`PP-ofertar-fe`), navegación con React Navigation (pila nativa, `src/navigation`) y estado compartido en stores de Zustand (`src/store`). Hasta la auditoría técnica de septiembre de 2026 eran una máquina de estados manual en `App.tsx` y `useState`; la auditoría lo marcó como el mayor riesgo de mantenimiento del frontend y se migró.
- Modelo de negocio planificado: **freemium**, definido en la hoja de supuestos del equipo ("Supuestos OfertAR — modelo económico", Google Sheets; los costos, ingresos y proyecciones viven ahí, no en este archivo). Plan **Gratis**: 3 tickets por mes, con anuncios. Plan **Premium**: $6.900 ARS por mes, 6 tickets por mes, sin anuncios (`src/data/plans.ts`). La hoja también menciona una "funcionalidad extra" solo Premium (hasta 3) cuyo nombre figura como `****`: sin definir, no inventarla. Nada de esto está implementado: no hay facturación ni suscripción, no existen los anuncios y el backend no limita hoy por plan (`ticket.max-per-user=50`). `PlansScreen` (Perfil → Planes) presenta estos planes como "próximamente" y dice que todavía no se cobra nada. La recompensa de "descuento de suscripción" del sistema de puntos sigue siendo ilustrativa hasta que exista el plan pago.
...
- Es un proyecto académico, no un producto en producción todavía. Features "post-MVP" (plan Premium, afiliados, modo offline) son terreno legítimo para explorar, pero no deben tratarse como compromisos comerciales reales sin que el usuario lo confirme explícitamente. Los planes, precios y límites son los de la hoja de supuestos: no inventar otros ni presentarlos como vigentes. Mientras no haya facturación, ninguna pantalla debe cobrar, prometer un límite que no rige ni mostrar un plan como "tu plan actual".
